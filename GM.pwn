...
CallBack::Relogio()
{
	...
	format(xString, sizeof xString, " %02d:%02d:%02d %02d/%02d/%d", hour[0], hour[1], hour[2], date[0], date[1], date[2]);
	TextDrawSetString(Interface[1], xString);
	return 1;
}
...
CallBack::PlayerUpdate(playerid)
{
	...
	for(new i = 0; i < DOF2_GetInt(file, "Ultimo ID"); i++)
	{
		if(rDados[i][rBloqueio] == 1)
		{
		    ...
		    {
		        format(xString, sizeof xString, "( Atenção Caminhoneiros ) A Rota %s acaba de ser liberada novamente!", i);
